...
- **Database and encryption:** EF Core owns schema migrations; Dapper performs parameterized FTS5 queries. Milestone 1 uses `Microsoft.Data.Sqlite` with ordinary SQLite. The provider boundary permits a SQLCipher-capable replacement, but encryption at rest is **not implemented** and must not be represented as complete.
...
The shared service supports multiple clients and central audit policy, while compromise of an MCP adapter does not grant database access. Loopback bearer authentication reduces—but does not eliminate—risk from another process running as the same OS user. SQLCipher integration, OS secret storage, token rotation, and replay resistance remain security work.
